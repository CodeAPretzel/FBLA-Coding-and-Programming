...
var GridRootPropsContext = React3.createContext(void 0);
if (true) {
...
var GridEditInputCell = React7.forwardRef((props, ref) => {
  ...
  const [valueState, setValueState] = React7.useState(value);
  const classes2 = useUtilityClasses(rootProps);
  const handleChange = React7.useCallback(async (event) => {
...
function GridFilterInputValue(props) {
  ...
  const [filterValueState, setFilterValueState] = React8.useState(item.value ?? "");
  const [applying, setIsApplying] = React8.useState(false);
  const id = useId();
  const rootProps = useGridRootProps();
  const onFilterChange = React8.useCallback((event) => {
    const {
      value
    } = event.target;
...
function GridFilterInputMultipleValue(props) {
  ...
  const handleChange = React9.useCallback((event, value) => {
...
function GridMenu(props) {
  ...
  const savedFocusRef = React10.useRef(null);
  useEnhancedEffect_default(() => {
    var _a2, _b;
    if (open) {
...
function GridActionsCell(props) {
  ...
  const [focusedButtonIndex, setFocusedButtonIndex] = React11.useState(-1);
  const [open, setOpen] = React11.useState(false);
  const apiRef = useGridApiContext();
  const rootRef = React11.useRef(null);
  const buttonRef = React11.useRef(null);
  const ignoreCallToFocus = React11.useRef(false);
  const touchRippleRefs = React11.useRef({});
  ...
  React11.useImperativeHandle(focusElementRef, () => ({
    focus() {
  ...
  return (0, import_jsx_runtime6.jsxs)("div", _extends({
    ...
    children: [iconButtons.map((button, index) => React11.cloneElement(button, {
      key: index,
      ...
      children: (0, import_jsx_runtime5.jsx)(MenuList_default, {
        ...
        variant: "menu",
        autoFocusItem: true,
        children: menuButtons.map((button, index) => React11.cloneElement(button, {
          key: index,
          closeMenu: hideMenu
        }))
      })
...
function GridFilterInputBoolean(props) {
  ...
  const [filterValueState, setFilterValueState] = React14.useState(item.value || "");
  const rootProps = useGridRootProps();
  const labelId = useId();
...
var GridCellCheckboxForwardRef = React15.forwardRef(function GridCellCheckboxRenderer(props, ref) {
  var _a;
  const {
    field,
    id,
  ...
  const checkboxElement = React15.useRef(null);
...
function useGridLogger(privateApiRef, name) {
  const logger = React17.useRef(null);
  if (logger.current) {
...
var useGridNativeEventListener = (apiRef, ref, eventName, handler, options) => {
  ...
  const [added, setAdded] = React18.useState(false);
  const handlerRef = React18.useRef(handler);
  const targetElement = isFunction(ref) ? ref() : (ref == null ? void 0 : ref.current) ?? null;
  const wrapHandler = React18.useCallback((event) => {
...
var useFirstRender = (callback) => {
  const isFirstRender = React19.useRef(true);
  if (isFirstRender.current) {
    isFirstRender.current = false;
    callback();
  }
...
function useResizeObserver(ref, fn, enabled) {
  const fnRef = React20.useRef(null);
  fnRef.current = fn;
  useEnhancedEffect_default(() => {
...
var GridPrivateApiContext = React30.createContext(void 0);
if (true) {
  GridPrivateApiContext.displayName = "GridPrivateApiContext";
}
function useGridPrivateApiContext() {
  const privateApiRef = React30.useContext(GridPrivateApiContext);
...
var useGridVirtualScroller = () => {
  ...
  const [panels, setPanels] = React33.useState(EMPTY_DETAIL_PANELS);
  const theme = useTheme();
  ...
  const scrollbarVerticalRef = React33.useRef(null);
  const scrollbarHorizontalRef = React33.useRef(null);
  const contentHeight = dimensions.contentSize.height;
  const columnsTotalWidth = dimensions.columnsTotalWidth;
  const hasColSpan = useGridSelector(apiRef, gridHasColSpanSelector);
  useResizeObserver(mainRef, () => apiRef.current.resize());
  const scrollPosition = React33.useRef(EMPTY_SCROLL_POSITION);
  const previousContextScrollPosition = React33.useRef(EMPTY_SCROLL_POSITION);
  const previousRowContext = React33.useRef(EMPTY_RENDER_CONTEXT);
  const renderContext = useGridSelector(apiRef, gridRenderContextSelector);
  const scrollTimeout = useTimeout();
  const frozenContext = React33.useRef(void 0);
  ...
  const focusedCell = {
    rowIndex: React33.useMemo(() => cellFocus ? currentPage.rows.findIndex((row) => row.id === cellFocus.id) : -1, [cellFocus, currentPage.rows]),
    columnIndex: React33.useMemo(() => cellFocus ? visibleColumns.findIndex((column) => column.field === cellFocus.field) : -1, [cellFocus, visibleColumns])
  };
  const updateRenderContext = React33.useCallback((nextRenderContext) => {
  ...
  const getRows = (params = {}) => {
    ...
    rowIndexes.forEach((rowIndexInPage) => {
      ...
      rows.push((0, import_jsx_runtime20.jsx)(rootProps.slots.row, _extends({
        row: model,
        rowId: id,
        index: rowIndex,
        selected: isSelected,
...
function GridOverlayWrapper(props) {
  ...
  const classes2 = useUtilityClasses8(_extends({}, props, {
    classes: rootProps.classes
  }));
  return (0, import_jsx_runtime21.jsx)(GridOverlayWrapperRoot, {
    className: clsx_default(classes2.root),
    overlayType: props.overlayType,
    children: (0, import_jsx_runtime21.jsx)(GridOverlayWrapperInner, _extends({
...
function GridOverlays() {
  ...
  if (showNoRowsOverlay) {
    overlay = (0, import_jsx_runtime21.jsx)(rootProps.slots.noRowsOverlay, _extends({}, (_a = rootProps.slotProps) == null ? void 0 : _a.noRowsOverlay));
    overlayType = "noRowsOverlay";
  }
  if (showNoResultsOverlay) {
    overlay = (0, import_jsx_runtime21.jsx)(rootProps.slots.noResultsOverlay, _extends({}, (_b = rootProps.slotProps) == null ? void 0 : _b.noResultsOverlay));
    overlayType = "noResultsOverlay";
  }
  if (loading) {
    overlay = (0, import_jsx_runtime21.jsx)(rootProps.slots.loadingOverlay, _extends({}, (_c = rootProps.slotProps) == null ? void 0 : _c.loadingOverlay));
    overlayType = "loadingOverlay";
  }
  if (overlay === null) {
    return null;
  }
  return (0, import_jsx_runtime21.jsx)(GridOverlayWrapper, {
    overlayType,
    children: overlay
  });
}
...
function GridHeaders() {
  ...
  const columnsContainerRef = React35.useRef(null);
  apiRef.current.register("private", {
    columnHeadersContainerRef: columnsContainerRef
  });
  return (0, import_jsx_runtime22.jsx)(rootProps.slots.columnHeaders, {
...
var useUtilityClasses9 = (props, overflowedContent) => {
  const {
    classes: classes2
  } = props;
  const slots = {
...
var GridVirtualScrollerContent = React36.forwardRef(function GridVirtualScrollerContent2(props, ref) {
  var _a;
  const rootProps = useGridRootProps();
  const overflowedContent = !rootProps.autoHeight && ((_a = props.style) == null ? void 0 : _a.minHeight) === "auto";
  const classes2 = useUtilityClasses9(rootProps, overflowedContent);
  return (0, import_jsx_runtime23.jsx)(VirtualScrollerContentRoot, _extends({
    ref
  }, props, {
    ownerState: rootProps,
...
var GridVirtualScrollerRenderZone = React37.forwardRef(function GridVirtualScrollerRenderZone2(props, ref) {
  const {
    className
  } = props, other = _objectWithoutPropertiesLoose(props, _excluded16);
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const classes2 = useUtilityClasses10(rootProps);
  const offsetTop = useGridSelector(apiRef, () => {
    const renderContext = gridRenderContextSelector(apiRef);
    const rowsMeta = gridRowsMetaSelector(apiRef.current.state);
    return rowsMeta.positions[renderContext.firstRowIndex] ?? 0;
  });
  return (0, import_jsx_runtime24.jsx)(VirtualScrollerRenderZoneRoot, _extends({
    ref,
    className: clsx_default(classes2.root, className),
    ownerState: rootProps,
    style: {
...
function GridScrollAreaRaw(props) {
  ...
  const dimensions = useGridSelector(apiRef, gridDimensionsSelector);
  const scrollPosition = React38.useRef({
    left: 0,
    top: 0
  });
  ...
  const [canScrollMore, setCanScrollMore] = React38.useState(getCanScrollMore);
  const rootProps = useGridRootProps();
  ...
  return (0, import_jsx_runtime25.jsx)(GridScrollAreaRawRoot, {
    ref: rootRef,
    className: clsx_default(classes2.root),
    ownerState,
...
true ? GridScrollAreaRaw.propTypes = {
  ...
  // | To update them edit the TypeScript types and run "yarn proptypes"  |
  // ----------------------------------------------------------------------
  scrollDirection: import_prop_types17.default.oneOf(["left", "right"]).isRequired
} : void 0;
...
var GridMainContainer = React39.forwardRef((props, ref) => {
  ...
  const rootProps = useGridRootProps();
  return (0, import_jsx_runtime26.jsx)(Element, _extends({
    ref,
    ownerState: rootProps,
...
function GridTopContainer(props) {
  ...
  return (0, import_jsx_runtime27.jsx)(Element2, _extends({}, props, {
...
function GridBottomContainer(props) {
  ...
  return (0, import_jsx_runtime28.jsx)(Element3, _extends({}, props, {
...
function GridVirtualScrollerFiller() {
  ...
  return (0, import_jsx_runtime30.jsxs)(Filler, {
...
var GridVirtualScrollbar = React43.forwardRef(function GridVirtualScrollbar2(props, ref) {
  const apiRef = useGridPrivateApiContext();
  const rootProps = useGridRootProps();
  const isLocked = React43.useRef(false);
  const lastPosition = React43.useRef(0);
  const scrollbarRef = React43.useRef(null);
...
function useGridParamsApi(apiRef) {
  const getColumnHeaderParams = React46.useCallback((field) => ({
    field,
    colDef: apiRef.current.getColumn(field)
  }), [apiRef]);
  const getRowParams = React46.useCallback((id) => {
  ...
  const getCellParams = React46.useCallback((id, field) => {
  ...
  const getCellValue = React46.useCallback((id, field) => {
    const colDef = apiRef.current.getColumn(field);
    const row = apiRef.current.getRow(id);
    if (!row) {
      throw new MissingRowIdError(`No row with id #${id} found`);
  ...
  const getRowValue = React46.useCallback((row, colDef) => {
  ...
  const getRowFormattedValue = React46.useCallback((row, colDef) => {
  ...
  const getColumnHeaderElement = React46.useCallback((field) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
    return getGridColumnHeaderElement(apiRef.current.rootElementRef.current, field);
  }, [apiRef]);
  const getRowElement = React46.useCallback((id) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
    return getGridRowElement(apiRef.current.rootElementRef.current, id);
  }, [apiRef]);
  const getCellElement = React46.useCallback((id, field) => {
    if (!apiRef.current.rootElementRef.current) {
      return null;
    }
...
var GridCell = React47.forwardRef((props, ref) => {
  var _a;
  const {
    column,
    rowId,
  ...
  const cellRef = React47.useRef(null);
  const handleRef = useForkRef(ref, cellRef);
  const focusElementRef = React47.useRef(null);
  ...
  const publishMouseUp = React47.useCallback((eventName) => (event) => {
  ...
  const publishMouseDown = React47.useCallback((eventName) => (event) => {
    const params = apiRef.current.getCellParams(rowId, field || "");
    apiRef.current.publishEvent(eventName, params, event);
    if (onMouseDown) {
      onMouseDown(event);
  ...
  const publish = React47.useCallback((eventName, propHandler) => (event) => {
  ...
  if (React47.isValidElement(children) && canManageOwnFocus) {
    children = React47.cloneElement(children, {
      focusElementRef
    });
  }
...
var GridActionsCellItem = React48.forwardRef((props, ref) => {
  var _a;
  const rootProps = useGridRootProps();
  if (!props.showInMenu) {
    const {
    ...
    return (0, import_jsx_runtime36.jsx)(rootProps.slots.baseIconButton, _extends({
      ...
      children: React48.cloneElement(icon2, {
        fontSize: "small"
      })
    }));
  }
...
var GridRoot = React50.forwardRef(function GridRoot2(props, ref) {
  const rootProps = useGridRootProps();
  const {
    children,
    className
...
var GridFooterContainer = React51.forwardRef(function GridFooterContainer2(props, ref) {
  const {
    className
...
var GridOverlay = React52.forwardRef(function GridOverlay2(props, ref) {
  const {
    className
...
var GridToolbarContainer = React53.forwardRef(function GridToolbarContainer2(props, ref) {
  const {
    className,
    children
...
var GridIconButtonContainer = React54.forwardRef(function GridIconButtonContainer2(props, ref) {
  const {
    className
...
var GridColumnHeaderSortIcon = React55.memo(GridColumnHeaderSortIconRaw);
...
function GridColumnHeaderMenu({
  ...
  return (0, import_jsx_runtime47.jsx)(GridMenu, {
    placement: `bottom-${colDef.align === "right" ? "start" : "end"}`,
    open,
    target,
    onClose: hideMenu,
    onExited,
    children: (0, import_jsx_runtime47.jsx)(ContentComponent, _extends({
      colDef,
      hideMenu,
      open,
      id: columnMenuId,
...
var ColumnHeaderInnerTitle = React57.forwardRef(function ColumnHeaderInnerTitle2(props, ref) {
  ...
  return (0, import_jsx_runtime48.jsx)(GridColumnHeaderTitleRoot, _extends({
    ref,
    className: clsx_default(classes2.root, className),
    ownerState: rootProps
  }, other));
...
function GridColumnHeaderTitle(props) {
  ...
  return (0, import_jsx_runtime48.jsx)(rootProps.slots.baseTooltip, _extends({
    title: description || tooltip
  }, (_a = rootProps.slotProps) == null ? void 0 : _a.baseTooltip, {
    children: (0, import_jsx_runtime48.jsx)(ColumnHeaderInnerTitle, {
...
function GridColumnHeaderSeparatorRaw(props) {
  ...
  return (
    ...
    (0, import_jsx_runtime49.jsx)("div", _extends({
      className: classes2.root,
      style: {
        minHeight: height,
      ...
      onClick: stopClick,
      children: (0, import_jsx_runtime49.jsx)(rootProps.slots.columnResizeIcon, {
...
var ColumnHeaderMenuIcon = React59.memo((props) => {
  ...
  return (0, import_jsx_runtime50.jsx)("div", {
    className: classes2.root,
    children: (0, import_jsx_runtime50.jsx)(rootProps.slots.baseTooltip, _extends({
      title: apiRef.current.getLocaleText("columnMenuLabel"),
      enterDelay: 1e3
    }, (_a = rootProps.slotProps) == null ? void 0 : _a.baseTooltip, {
      children: (0, import_jsx_runtime50.jsx)(rootProps.slots.baseIconButton, _extends({
        ref: iconButtonRef,
        tabIndex: -1,
        ...
        children: (0, import_jsx_runtime50.jsx)(rootProps.slots.columnMenuIcon, {
          fontSize: "small"
        })
      }))
    }))
...
var GridGenericColumnHeaderItem = React60.forwardRef(function GridGenericColumnHeaderItem2(props, ref) {
  const {
    classes: classes2,
    columnMenuOpen,
    colIndex,
...
function GridColumnHeaderItem(props) {
  ...
  const iconButtonRef = React61.useRef(null);
  const [showColumnMenuIcon, setShowColumnMenuIcon] = React61.useState(columnMenuOpen);
  const isDraggable = React61.useMemo(() => !rootProps.disableColumnReorder && !disableReorder && !colDef.disableReorder, [rootProps.disableColumnReorder, disableReorder, colDef.disableReorder]);
  let headerComponent;
  ...
  const publish = React61.useCallback((eventName) => (event) => {
    if (isEventTargetInPortal(event)) {
      return;
    }
    apiRef.current.publishEvent(eventName, apiRef.current.getColumnHeaderParams(colDef.field), event);
  }, [apiRef, colDef.field]);
  const mouseEventsHandlers = React61.useMemo(() => ({
  ...
  const draggableEventHandlers = React61.useMemo(() => isDraggable ? {
    onDragStart: publish("columnHeaderDragStart"),
    onDragEnter: publish("columnHeaderDragEnter"),
    onDragOver: publish("columnHeaderDragOver"),
    onDragEnd: publish("columnHeaderDragEnd")
  } : {}, [isDraggable, publish]);
  const columnHeaderSeparatorProps = React61.useMemo(() => ({
  ...
  const handleExited = React61.useCallback(() => {
  ...
  const columnTitleIconButtons = (0, import_jsx_runtime54.jsxs)(React61.Fragment, {
...
function GridColumnHeaderFilterIconButton(props) {
  ...
  const labelId = useId();
  const panelId = useId();
  const toggleFilter = React62.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    const {
      open: open2,
...
var GridColumnMenuContainer = React64.forwardRef(function GridColumnMenuContainer2(props, ref) {
  const {
    hideMenu,
    id,
    labelledby,
  ...
  const handleListKeyDown = React64.useCallback((event) => {
...
function GridColumnMenuHideItem(props) {
  ...
  const toggleColumn = React65.useCallback((event) => {
    if (disabled) {
      return;
    }
...
function GridColumnMenuManageItem(props) {
  ...
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const showColumns = React66.useCallback((event) => {
    onClick(event);
...
function GridColumnMenuFilterItem(props) {
  ...
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const showFilter = React68.useCallback((event) => {
    onClick(event);
...
function GridColumnMenuSortItem(props) {
  ...
  const rootProps = useGridRootProps();
  const sortDirection = React69.useMemo(() => {
    if (!colDef) {
      return null;
    }
  ...
  const sortingOrder = colDef.sortingOrder ?? rootProps.sortingOrder;
  const onSortMenuItemClick = React69.useCallback((event) => {
    onClick(event);
  ...
  return (0, import_jsx_runtime68.jsxs)(React69.Fragment, {
...
var GridGenericColumnMenu = React71.forwardRef(function GridGenericColumnMenu2(props, ref) {
  const {
    defaultSlots: defaultSlots2,
    defaultSlotProps,
    slots,
...
var GridColumnMenu = React71.forwardRef(function GridColumnMenu2(props, ref) {
...
var isEnabled = () => true;
var GridPanelWrapper = React72.forwardRef(function GridPanelWrapper2(props, ref) {
  const {
    className,
    slotProps = {}
...
function GridColumnsManagement(props) {
  var _a, _b, _c;
  const apiRef = useGridApiContext();
  const searchInputRef = React73.useRef(null);
  ...
  const rootProps = useGridRootProps();
  const [searchValue, setSearchValue] = React73.useState("");
  ...
  const isResetDisabled = React73.useMemo(() => checkColumnVisibilityModelsSame(columnVisibilityModel, initialColumnVisibilityModel), [columnVisibilityModel, initialColumnVisibilityModel]);
  const sortedColumns = React73.useMemo(() => {
    switch (sort) {
      case "asc":
  ...
  const currentColumns = React73.useMemo(() => {
  ...
  const toggleAllColumns = React73.useCallback((isVisible) => {
  ...
  const handleSearchValueChange = React73.useCallback((event) => {
    setSearchValue(event.target.value);
  }, []);
  const hideableColumns = React73.useMemo(() => currentColumns.filter((col) => col.hideable), [currentColumns]);
  const allHideableColumnsVisible = React73.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] == null || columnVisibilityModel[column.field] !== false), [columnVisibilityModel, hideableColumns]);
  const allHideableColumnsHidden = React73.useMemo(() => hideableColumns.every((column) => columnVisibilityModel[column.field] === false), [columnVisibilityModel, hideableColumns]);
  const firstSwitchRef = React73.useRef(null);
  React73.useEffect(() => {
...
var GridToolbarColumnsButton = React81.forwardRef(function GridToolbarColumnsButton2(props, ref) {
  var _a, _b;
  const {
    slotProps = {}
  } = props;
...
var GridToolbarDensitySelector = React82.forwardRef(function GridToolbarDensitySelector2(props, ref) {
  var _a, _b;
  const {
    slotProps = {}
  } = props;
  ...
  const startIcon = React82.useMemo(() => {
    switch (density) {
      case "compact":
  ...
  return (0, import_jsx_runtime84.jsxs)(React82.Fragment, {
...
var GridToolbarFilterButton = React83.forwardRef(function GridToolbarFilterButton2(props, ref) {
  var _a, _b;
  const {
    slotProps = {}
  } = props;
  ...
  const tooltipContentNode = React83.useMemo(() => {
...
var GridToolbarExportContainer = React84.forwardRef(function GridToolbarExportContainer2(props, ref) {
  var _a, _b;
  const {
    children,
    slotProps = {}
...
var GridToolbarExport = React85.forwardRef(function GridToolbarExport2(props, ref) {
  const {
    csvOptions = {},
    printOptions = {},
    excelOptions
  ...
  return (0, import_jsx_runtime89.jsx)(GridToolbarExportContainer, _extends({}, other, {
    ref,
    children: preProcessedButtons.map((button, index) => React85.cloneElement(button.component, {
      key: index
    }))
  }));
});
...
function GridToolbarQuickFilter(props) {
  ...
  const [searchValue, setSearchValue] = React86.useState(() => quickFilterFormatter(quickFilterValues ?? []));
  const prevQuickFilterValuesRef = React86.useRef(quickFilterValues);
  React86.useEffect(() => {
  ...
  const updateSearchValue = React86.useCallback((newSearchValue) => {
    const newQuickFilterValues = quickFilterParser(newSearchValue);
    prevQuickFilterValuesRef.current = newQuickFilterValues;
    apiRef.current.setQuickFilterValues(newQuickFilterValues);
  }, [apiRef, quickFilterParser]);
  const debouncedUpdateSearchValue = React86.useMemo(() => debounce(updateSearchValue, debounceMs), [updateSearchValue, debounceMs]);
...
var GridToolbar = React87.forwardRef(function GridToolbar2(props, ref) {
  const {
    csvOptions,
    printOptions,
    excelOptions,
...
var GridSelectedRowCount = React88.forwardRef(function GridSelectedRowCount2(props, ref) {
  const {
    className,
    selectedRowCount
...
var GridFooter = React89.forwardRef(function GridFooter2(props, ref) {
  var _a, _b;
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
...
var GridPreferencesPanel = React90.forwardRef(function GridPreferencesPanel2(props, ref) {
  var _a, _b;
  const apiRef = useGridApiContext();
  const columns = useGridSelector(apiRef, gridColumnDefinitionsSelector);
  const rootProps = useGridRootProps();
...
function GridHeader() {
  var _a;
  const rootProps = useGridRootProps();
  return (0, import_jsx_runtime98.jsxs)(React91.Fragment, {
...
var GridLoadingOverlay = React92.forwardRef(function GridLoadingOverlay2(props, ref) {
...
var GridNoRowsOverlay = React93.forwardRef(function GridNoRowsOverlay2(props, ref) {
  const apiRef = useGridApiContext();
...
var GridPagination = React94.forwardRef(function GridPagination2(props, ref) {
  var _a;
  const apiRef = useGridApiContext();
  const rootProps = useGridRootProps();
  const paginationModel = useGridSelector(apiRef, gridPaginationModelSelector);
  const rowCount = useGridSelector(apiRef, gridPaginationRowCountSelector);
  const lastPage = React94.useMemo(() => {
    const calculatedValue = Math.ceil(rowCount / (paginationModel.pageSize || 1)) - 1;
    return Math.max(0, calculatedValue);
  }, [rowCount, paginationModel.pageSize]);
  const handlePageSizeChange = React94.useCallback((event) => {
    const pageSize = Number(event.target.value);
    apiRef.current.setPageSize(pageSize);
  }, [apiRef]);
  const handlePageChange = React94.useCallback((_, page) => {
  ...
  if (true) {
    const warnedOnceMissingInPageSizeOptions = React94.useRef(false);
...
var GridRowCount = React95.forwardRef(function GridRowCount2(props, ref) {
  const {
    className,
    rowCount,
    visibleRowCount
...
var GridRow = React97.forwardRef(function GridRow2(props, refProp) {
  var _a;
  const {
    selected,
    rowId,
  ...
  const publish = React97.useCallback((eventName, propHandler) => (event) => {
    if (isEventTargetInPortal(event)) {
      return;
    }
    if (!apiRef.current.getRow(rowId)) {
  ...
  const publishClick = React97.useCallback((event) => {
...
function useGridApiInitialization(inputApiRef, props) {
  var _a;
  const publicApiRef = React98.useRef();
  const privateApiRef = React98.useRef();
  ...
  const publishEvent = React98.useCallback((...args) => {
  ...
  const subscribeEvent = React98.useCallback((event, handler, options) => {
  ...
  React98.useImperativeHandle(inputApiRef, () => publicApiRef.current, [publicApiRef]);
  React98.useEffect(() => {
    const api = privateApiRef.current;
    return () => {
...
var useGridRegisterPipeProcessor = (apiRef, group, callback) => {
  const cleanup = React99.useRef();
  const id = React99.useRef(`mui-${Math.round(Math.random() * 1e9)}`);
  const registerPreProcessor = React99.useCallback(() => {
...
var useGridPipeProcessing = (apiRef) => {
  const cache2 = React100.useRef({});
  const isRunning = React100.useRef(false);
  const runAppliers = React100.useCallback((groupCache) => {
  ...
  const registerPipeProcessor = React100.useCallback((group, id, processor) => {
  ...
  const registerPipeApplier = React100.useCallback((group, id, applier) => {
    if (!cache2.current[group]) {
      cache2.current[group] = {
        processors: /* @__PURE__ */ new Map(),
        processorsAsArray: [],
  ...
  const requestPipeProcessorsApplication = React100.useCallback((group) => {
    runAppliers(cache2.current[group]);
  }, [runAppliers]);
  const applyPipeProcessors = React100.useCallback((...args) => {
...
var useGridStrategyProcessing = (apiRef) => {
  const availableStrategies = React103.useRef(/* @__PURE__ */ new Map());
  const strategiesCache = React103.useRef({});
  const registerStrategyProcessor = React103.useCallback((strategyName, processorName, processor) => {
    const cleanup = () => {
  ...
  const applyStrategyProcessor = React103.useCallback((processorName, params) => {
  ...
  const getActiveStrategy = React103.useCallback((strategyGroup) => {
  ...
  const setStrategyAvailability = React103.useCallback((strategyGroup, strategyName, isAvailable) => {
...
var useGridStateInitialization = (apiRef, props) => {
  const controlStateMapRef = React108.useRef({});
  const [, rawForceUpdate] = React108.useState();
  const registerControlState = React108.useCallback((controlStateItem) => {
    controlStateMapRef.current[controlStateItem.stateId] = controlStateItem;
  }, []);
  const setState = React108.useCallback((state, reason) => {
    let newState;
    if (isFunction(state)) {
  ...
  const updateControlState = React108.useCallback((key, state, reason) => {
  ...
  const forceUpdate = React108.useCallback(() => rawForceUpdate(() => apiRef.current.state), [apiRef]);
...
var useGridInitializeState = (initializer, privateApiRef, props) => {
  const isInitialized = React109.useRef(false);
...
function useGridColumns(apiRef, props) {
  ...
  const preferencePanelPreProcessing = React112.useCallback((initialValue, value) => {
    ...
    if (value === GridPreferencePanelsValue.columns) {
      ...
      return (0, import_jsx_runtime106.jsx)(ColumnsPanel, _extends({}, (_a2 = props.slotProps) == null ? void 0 : _a2.columnsPanel));
    }
    return initialValue;
...
var useGridCsvExport = (apiRef, props) => {
  ...
  const getDataAsCsv = React114.useCallback((options = {}) => {
  ...
  const exportDataAsCsv = React114.useCallback((options) => {
...
var useGridPaginationModel = (apiRef, props) => {
  ...
  const setPage = React115.useCallback((page) => {
  ...
  const setPageSize = React115.useCallback((pageSize) => {
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const setPaginationModel = React115.useCallback((paginationModel) => {
    const currentModel = gridPaginationModelSelector(apiRef);
  ...
  const handleUpdateAutoPageSize = React115.useCallback(() => {
  ...
  const handleRowCountChange = React115.useCallback((newRowCount) => {
    if (newRowCount == null) {
      return;
    }
    const paginationModel = gridPaginationModelSelector(apiRef);
  ...
  React115.useEffect(handleUpdateAutoPageSize, [handleUpdateAutoPageSize]);
};
...
var useGridPrintExport = (apiRef, props) => {
  ...
  const doc = React116.useRef(null);
  const previousGridState = React116.useRef(null);
  const previousColumnVisibility = React116.useRef({});
  const previousRows = React116.useRef([]);
  React116.useEffect(() => {
    doc.current = ownerDocument(apiRef.current.rootElementRef.current);
  }, [apiRef]);
  const updateGridColumnsForPrint = React116.useCallback((fields, allColumns, includeCheckboxes) => new Promise((resolve) => {
  ...
  const updateGridRowsForPrint = React116.useCallback((getRowsToExport) => {
  ...
  const handlePrintWindowLoad = React116.useCallback((printWindow, options) => {
    var _a, _b, _c, _d;
...
`;
  ...
  const handlePrintWindowAfterPrint = React116.useCallback((printWindow) => {
    var _a, _b;
  ...
  const exportDataAsPrint = React116.useCallback(async (options) => {
...
var useGridFilter = (apiRef, props) => {
  ...
  const updateFilteredRows = React117.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const applyFilters = React117.useCallback(() => {
    updateFilteredRows();
    apiRef.current.forceUpdate();
  }, [apiRef, updateFilteredRows]);
  const upsertFilterItem = React117.useCallback((item) => {
  ...
  const upsertFilterItems = React117.useCallback((items) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const deleteFilterItem = React117.useCallback((itemToDelete) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const showFilterPanel = React117.useCallback((targetColumnField, panelId, labelId) => {
  ...
  const hideFilterPanel = React117.useCallback(() => {
    logger.debug("Hiding filter panel");
    apiRef.current.hidePreferences();
  }, [apiRef, logger]);
  const setFilterLogicOperator = React117.useCallback((logicOperator) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setQuickFilterValues = React117.useCallback((values) => {
    const filterModel = gridFilterModelSelector(apiRef);
  ...
  const setFilterModel = React117.useCallback((model, reason) => {
  ...
  const preferencePanelPreProcessing = React117.useCallback((initialValue, value) => {
    ...
    if (value === GridPreferencePanelsValue.filters) {
      ...
      return (0, import_jsx_runtime109.jsx)(FilterPanel, _extends({}, (_a2 = props.slotProps) == null ? void 0 : _a2.filterPanel));
    }
    return initialValue;
  ...
  const flatFilteringMethod = React117.useCallback((params) => {
  ...
  const updateVisibleRowsLookupState = React117.useCallback(() => {
    apiRef.current.setState((state) => {
      return _extends({}, state, {
...
var useGridFocus = (apiRef, props) => {
  ...
  const lastClickedCell = React118.useRef(null);
  const publishCellFocusOut = React118.useCallback((cell, event) => {
    if (cell) {
  ...
  const setCellFocus = React118.useCallback((id, field) => {
    const focusedCell = gridFocusCellSelector(apiRef);
  ...
  const setColumnHeaderFocus = React118.useCallback((field, event = {}) => {
  ...
  const setColumnHeaderFilterFocus = React118.useCallback((field, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    publishCellFocusOut(cell, event);
    apiRef.current.setState((state) => {
  ...
  const setColumnGroupHeaderFocus = React118.useCallback((field, depth, event = {}) => {
    const cell = gridFocusCellSelector(apiRef);
    if (cell) {
      apiRef.current.publishEvent("cellFocusOut", apiRef.current.getCellParams(cell.id, cell.field), event);
    }
  ...
  const getColumnGroupHeaderFocus = React118.useCallback(() => gridFocusColumnGroupHeaderSelector(apiRef), [apiRef]);
  const moveFocusToRelativeCell = React118.useCallback((id, field, direction) => {
  ...
  const handleCellDoubleClick = React118.useCallback(({
    id,
    field
  }) => {
  ...
  const handleColumnHeaderFocus = React118.useCallback(({
    field
  }, event) => {
  ...
  const handleColumnGroupHeaderFocus = React118.useCallback(({
    fields,
    depth
  }, event) => {
    if (event.target !== event.currentTarget) {
  ...
  const handleBlur = React118.useCallback((_, event) => {
    var _a;
  ...
  const handleCellMouseDown = React118.useCallback((params) => {
    lastClickedCell.current = params;
  }, []);
  const handleDocumentClick = React118.useCallback((event) => {
  ...
  const handleCellModeChange = React118.useCallback((params) => {
  ...
  const handleRowSet = React118.useCallback(() => {
    const cell = gridFocusCellSelector(apiRef);
...
var useGridKeyboardNavigation = (apiRef, props) => {
  ...
  const theme = useTheme();
  const currentPageRows = React119.useMemo(() => enrichPageRowsWithPinnedRows(apiRef, initialCurrentPageRows), [apiRef, initialCurrentPageRows]);
  const headerFilteringEnabled = props.signature !== "DataGrid" && props.headerFilters;
  const goToCell = React119.useCallback((colIndex, rowId, closestColumnToUse = "left") => {
  ...
  const goToHeader = React119.useCallback((colIndex, event) => {
  ...
  const goToHeaderFilter = React119.useCallback((colIndex, event) => {
  ...
  const goToGroupHeader = React119.useCallback((colIndex, depth, event) => {
    logger.debug(`Navigating to header col ${colIndex}`);
    apiRef.current.scrollToIndexes({
      colIndex
    });
  ...
  const getRowIdFromIndex = React119.useCallback((rowIndex) => {
    var _a;
    return (_a = currentPageRows[rowIndex]) == null ? void 0 : _a.id;
  }, [currentPageRows]);
  const handleColumnHeaderKeyDown = React119.useCallback((params, event) => {
  ...
  const handleHeaderFilterKeyDown = React119.useCallback((params, event) => {
  ...
  const handleColumnGroupHeaderKeyDown = React119.useCallback((params, event) => {
    const focusedColumnGroup = gridFocusColumnGroupHeaderSelector(apiRef);
  ...
  const checkIfCanStartEditing = React119.useCallback((initialValue, {
    event
  }) => {
    if (event.key === " ") {
      return false;
...
var useGridRowCount = (apiRef, props) => {
  ...
  const setRowCount = React120.useCallback((newRowCount) => {
...
var useGridPreferencesPanel = (apiRef, props) => {
  ...
  const hideTimeout = React121.useRef();
  const immediateTimeout = React121.useRef();
  const hidePreferences = React121.useCallback(() => {
  ...
  const doNotHidePanel = React121.useCallback(() => {
    immediateTimeout.current = setTimeout(() => clearTimeout(hideTimeout.current), 0);
  }, []);
  const hidePreferencesDelayed = React121.useCallback(() => {
    hideTimeout.current = setTimeout(hidePreferences, 100);
  }, [hidePreferences]);
  const showPreferences = React121.useCallback((newValue, panelId, labelId) => {
...
var useGridCellEditing = (apiRef, props) => {
  const [cellModesModel, setCellModesModel] = React122.useState({});
  const cellModesModelRef = React122.useRef(cellModesModel);
  const prevCellModesModel = React122.useRef({});
  const {
    processRowUpdate,
  ...
  const throwIfNotEditable = React122.useCallback((id, field) => {
    const params = apiRef.current.getCellParams(id, field);
    if (!apiRef.current.isCellEditable(params)) {
      throw new Error(`MUI X: The cell with id=${id} and field=${field} is not editable.`);
    }
  }, [apiRef]);
  const throwIfNotInMode = React122.useCallback((id, field, mode) => {
    if (apiRef.current.getCellMode(id, field) !== mode) {
      throw new Error(`MUI X: The cell with id=${id} and field=${field} is not in ${mode} mode.`);
    }
  }, [apiRef]);
  const handleCellDoubleClick = React122.useCallback((params, event) => {
  ...
  const handleCellFocusOut = React122.useCallback((params, event) => {
  ...
  const handleCellEditStart = React122.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleCellEditStop = React122.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const getCellMode = React122.useCallback((id, field) => {
  ...
  const updateFieldInCellModesModel = React122.useCallback((id, field, newProps) => {
  ...
  const updateOrDeleteFieldState = React122.useCallback((id, field, newProps) => {
    apiRef.current.setState((state) => {
  ...
  const startCellEditMode = React122.useCallback((params) => {
    const {
      id,
      field
  ...
  const stopCellEditMode = React122.useCallback((params) => {
    const {
      id,
      field
  ...
  const setCellEditingEditCellValue = React122.useCallback(async (params) => {
    var _a, _b;
    const {
      id,
      field,
  ...
  const getRowWithUpdatedValuesFromCellEditing = React122.useCallback((id, field) => {
    const column = apiRef.current.getColumn(field);
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
...
var useGridRowEditing = (apiRef, props) => {
  const [rowModesModel, setRowModesModel] = React123.useState({});
  const rowModesModelRef = React123.useRef(rowModesModel);
  const prevRowModesModel = React123.useRef({});
  const focusTimeout = React123.useRef();
  const nextFocusedCell = React123.useRef(null);
  const {
    processRowUpdate,
    onProcessRowUpdateError,
  ...
  const throwIfNotInMode = React123.useCallback((id, mode) => {
    if (apiRef.current.getRowMode(id) !== mode) {
      throw new Error(`MUI X: The row with id=${id} is not in ${mode} mode.`);
    }
  }, [apiRef]);
  const hasFieldsWithErrors = React123.useCallback((rowId) => {
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
  ...
  const handleCellFocusIn = React123.useCallback((params) => {
  ...
  const handleRowEditStart = React123.useCallback((params) => {
    const {
      id,
      field,
      reason
  ...
  const handleRowEditStop = React123.useCallback((params) => {
    const {
      id,
      reason,
      field
  ...
  const getRowMode = React123.useCallback((id) => {
    if (props.editMode === GridEditModes.Cell) {
  ...
  const updateRowInRowModesModel = React123.useCallback((id, newProps) => {
  ...
  const updateOrDeleteRowState = React123.useCallback((id, newProps) => {
    apiRef.current.setState((state) => {
      const newEditingState = _extends({}, state.editRows);
      if (newProps !== null) {
  ...
  const startRowEditMode = React123.useCallback((params) => {
    const {
      id
  ...
  const stopRowEditMode = React123.useCallback((params) => {
    const {
      id
  ...
  const setRowEditingEditCellValue = React123.useCallback((params) => {
    const {
      id,
      field,
      value,
  ...
  const getRowWithUpdatedValuesFromRowEditing = React123.useCallback((id) => {
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
    const row = apiRef.current.getRow(id);
    if (!editingState[id]) {
      return apiRef.current.getRow(id);
...
var useGridEditing = (apiRef, props) => {
  ...
  const debounceMap = React124.useRef({});
  const {
    isCellEditable: isCellEditableProp
  } = props;
  const isCellEditable = React124.useCallback((params) => {
  ...
  const runPendingEditCellValueMutation = React124.useCallback((id, field) => {
    if (!debounceMap.current[id]) {
      return;
    }
    if (!field) {
  ...
  const setEditCellValue = React124.useCallback((params) => {
    const {
      id,
      field,
      debounceMs
  ...
  const getRowWithUpdatedValues = React124.useCallback((id, field) => {
    return props.editMode === GridEditModes.Cell ? apiRef.current.getRowWithUpdatedValuesFromCellEditing(id, field) : apiRef.current.getRowWithUpdatedValuesFromRowEditing(id);
  }, [apiRef, props.editMode]);
  const getEditCellMeta = React124.useCallback((id, field) => {
    var _a;
    const editingState = gridEditRowsStateSelector(apiRef.current.state);
...
var useGridRows = (apiRef, props) => {
  ...
  const lastUpdateMs = React125.useRef(Date.now());
  const timeout = useTimeout();
  const getRow = React125.useCallback((id) => {
  ...
  const getRowId = React125.useCallback((row) => {
  ...
  const lookup = React125.useMemo(() => currentPage.rows.reduce((acc, {
    id
  }, index) => {
    acc[id] = index;
    return acc;
  }, {}), [currentPage.rows]);
  const throttledRowsChange = React125.useCallback(({
    cache: cache2,
    throttle: throttle2
  }) => {
    const run = () => {
  ...
  const setRows = React125.useCallback((rows) => {
  ...
  const updateRows = React125.useCallback((updates) => {
  ...
  const getRowModels = React125.useCallback(() => {
    const dataRows = gridDataRowIdsSelector(apiRef);
    const idRowsLookup = gridRowsLookupSelector(apiRef);
    return new Map(dataRows.map((id) => [id, idRowsLookup[id] ?? {}]));
  }, [apiRef]);
  const getRowsCount = React125.useCallback(() => gridRowCountSelector(apiRef), [apiRef]);
  const getAllRowIds = React125.useCallback(() => gridDataRowIdsSelector(apiRef), [apiRef]);
  const getRowIndexRelativeToVisibleRows = React125.useCallback((id) => lookup[id], [lookup]);
  const setRowChildrenExpansion = React125.useCallback((id, isExpanded) => {
  ...
  const getRowNode = React125.useCallback((id) => gridRowTreeSelector(apiRef)[id] ?? null, [apiRef]);
  const getRowGroupChildren = React125.useCallback(({
  ...
  const setRowIndex = React125.useCallback((rowId, targetIndex) => {
  ...
  const replaceRows = React125.useCallback((firstRowToRender, newRows) => {
  ...
  const groupRows = React125.useCallback(() => {
  ...
  const handleStrategyActivityChange = React125.useCallback(() => {
  ...
  const applyHydrateRowsProcessor = React125.useCallback(() => {
    apiRef.current.setState((state) => {
...
var useGridRowSelection = (apiRef, props) => {
  ...
  const propRowSelectionModel = React126.useMemo(() => {
    return getSelectionModelPropValue(props.rowSelectionModel, gridRowSelectionStateSelector(apiRef.current.state));
  }, [apiRef, props.rowSelectionModel]);
  const lastRowToggled = React126.useRef(null);
  apiRef.current.registerControlState({
  ...
  const expandMouseRowRangeSelection = React126.useCallback((id) => {
    let endId = id;
  ...
  const setRowSelectionModel = React126.useCallback((model) => {
  ...
  const isRowSelected = React126.useCallback((id) => gridRowSelectionStateSelector(apiRef.current.state).includes(id), [apiRef]);
  const isRowSelectable = React126.useCallback((id) => {
  ...
  const getSelectedRows = React126.useCallback(() => selectedGridRowsSelector(apiRef), [apiRef]);
  const selectRow = React126.useCallback((id, isSelected = true, resetSelection = false) => {
  ...
  const selectRows = React126.useCallback((ids, isSelected = true, resetSelection = false) => {
  ...
  const selectRowRange = React126.useCallback(({
    startId,
    endId
  ...
  const removeOutdatedSelection = React126.useCallback(() => {
  ...
  const handleSingleRowSelection = React126.useCallback((id, event) => {
  ...
  const handleRowClick = React126.useCallback((params, event) => {
    var _a;
  ...
  const preventSelectionOnShift = React126.useCallback((params, event) => {
    var _a;
    if (canHaveMultipleSelection && event.shiftKey) {
      (_a = window.getSelection()) == null ? void 0 : _a.removeAllRanges();
    }
  }, [canHaveMultipleSelection]);
  const handleRowSelectionCheckboxChange = React126.useCallback((params, event) => {
  ...
  const handleHeaderSelectionCheckboxChange = React126.useCallback((params) => {
...
var useGridRowSelectionPreProcessors = (apiRef, props) => {
  ...
  const updateSelectionColumn = React127.useCallback((columnsState) => {
...
var useGridSorting = (apiRef, props) => {
  ...
  const upsertSortModel = React128.useCallback((field, sortItem) => {
  ...
  const createSortItem = React128.useCallback((col, directionOverride) => {
    const sortModel = gridSortModelSelector(apiRef);
  ...
  const addColumnMenuItem = React128.useCallback((columnMenuItems, colDef) => {
  ...
  const applySorting = React128.useCallback(() => {
    apiRef.current.setState((state) => {
  ...
  const setSortModel = React128.useCallback((model) => {
  ...
  const sortColumn = React128.useCallback((field, direction, allowMultipleSorting) => {
    const column = apiRef.current.getColumn(field);
  ...
  const getSortModel = React128.useCallback(() => gridSortModelSelector(apiRef), [apiRef]);
  const getSortedRows = React128.useCallback(() => {
    const sortedRows = gridSortedRowEntriesSelector(apiRef);
    return sortedRows.map((row) => row.model);
  }, [apiRef]);
  const getSortedRowIds = React128.useCallback(() => gridSortedRowIdsSelector(apiRef), [apiRef]);
  const getRowIdFromRowIndex = React128.useCallback((index) => apiRef.current.getSortedRowIds()[index], [apiRef]);
  const sortApi = {
    getSortModel,
    getSortedRows,
    getSortedRowIds,
  ...
  const flatSortingMethod = React128.useCallback((params) => {
    const rowTree = gridRowTreeSelector(apiRef);
  ...
  const handleColumnHeaderClick = React128.useCallback(({
    field,
    colDef
  }, event) => {
  ...
  const handleColumnHeaderKeyDown = React128.useCallback(({
    field,
    colDef
  }, event) => {
    if (!colDef.sortable || props.disableColumnSorting) {
  ...
  const handleColumnsChange = React128.useCallback(() => {
    const sortModel = gridSortModelSelector(apiRef);
...
var useGridScroll = (apiRef, props) => {
  ...
  const scrollToIndexes = React129.useCallback((params) => {
    var _a;
    const dimensions = gridDimensionsSelector(apiRef.current.state);
  ...
  const scroll = React129.useCallback((params) => {
  ...
  const getScrollPosition = React129.useCallback(() => {
...
function useGridDimensions(apiRef, props) {
  ...
  const errorShown = React130.useRef(false);
  const rootDimensionsRef = React130.useRef(EMPTY_SIZE);
  const rowsMeta = useGridSelector(apiRef, gridRowsMetaSelector);
  const pinnedColumns = useGridSelector(apiRef, gridVisiblePinnedColumnDefinitionsSelector);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const rowHeight = Math.floor(props.rowHeight * densityFactor);
  ...
  const [savedSize, setSavedSize] = React130.useState();
  const debouncedSetSavedSize = React130.useMemo(() => throttle(setSavedSize, props.resizeThrottleMs), [props.resizeThrottleMs]);
  const previousSize = React130.useRef();
  ...
  const resize = React130.useCallback(() => {
    var _a, _b;
  ...
  const getViewportPageSize = React130.useCallback(() => {
  ...
  const updateDimensions = React130.useCallback(() => {
    var _a, _b;
  ...
  const isFirstSizing = React130.useRef(true);
  const handleResize = React130.useCallback((size) => {
...
function GridColumnGroupHeader(props) {
  ...
  const rootProps = useGridRootProps();
  const headerCellRef = React131.useRef(null);
  const apiRef = useGridApiContext();
  ...
  const renderParams = React131.useMemo(() => ({
    groupId,
    headerName,
    description,
    depth,
  ...
  const publish = React131.useCallback(
    (eventName) => (event) => {
      if (isEventTargetInPortal(event)) {
        return;
      }
  ...
  const mouseEventsHandlers = React131.useMemo(() => ({
    onKeyDown: publish("columnGroupHeaderKeyDown"),
    onFocus: publish("columnGroupHeaderFocus"),
    onBlur: publish("columnGroupHeaderBlur")
  }), [publish]);
  const headerClassName = typeof group.headerClassName === "function" ? group.headerClassName(renderParams) : group.headerClassName;
  return (0, import_jsx_runtime110.jsx)(GridGenericColumnHeaderItem, _extends({
    ref: headerCellRef,
    classes: classes2,
...
var useGridColumnHeaders = (props) => {
  ...
  const leftRenderContext = React132.useMemo(() => {
    return pinnedColumns.left.length ? {
      firstColumnIndex: 0,
      lastColumnIndex: pinnedColumns.left.length
    } : null;
  }, [pinnedColumns.left.length]);
  const rightRenderContext = React132.useMemo(() => {
  ...
  const getFillers = (params, children, leftOverflow, borderTop = false) => {
    ...
    return (0, import_jsx_runtime112.jsxs)(React132.Fragment, {
      children: [isNotPinned && (0, import_jsx_runtime111.jsx)("div", {
        role: "presentation",
        style: {
        ...
        role: "presentation",
        className: gridClasses.filler
      }), hasScrollbarFiller && (0, import_jsx_runtime111.jsx)(GridScrollbarFillerCell, {
        header: true,
        borderTop,
  ...
  const getColumnHeaders = (params, other = {}) => {
    ...
    for (let i = 0; i < renderedColumns.length; i += 1) {
      ...
      columns.push((0, import_jsx_runtime111.jsx)(Memoized3, _extends({}, sortColumnLookup[colDef.field], {
        columnMenuOpen: open,
  ...
  const getColumnHeadersRow = () => {
    return (0, import_jsx_runtime112.jsxs)(GridColumnHeaderRow, {
      role: "row",
  ...
  const getColumnGroupHeaders = ({
    ...
    const children = visibleColumnGroupHeader.map(({
      ...
      return (0, import_jsx_runtime111.jsx)(GridColumnGroupHeader, {
        groupId,
  ...
  const getColumnGroupHeadersRows = () => {
    ...
    for (let depth = 0; depth < headerGroupingMaxDepth; depth += 1) {
      headerRows.push((0, import_jsx_runtime112.jsxs)(GridColumnHeaderRow, {
        role: "row",
...
var GridBaseColumnHeaders = React133.forwardRef(function GridColumnHeaders(props, ref) {
  const {
    className
  } = props, other = _objectWithoutPropertiesLoose(props, _excluded49);
  const rootProps = useGridRootProps();
  const classes2 = useUtilityClasses42(rootProps);
  return (0, import_jsx_runtime113.jsx)(GridColumnHeadersRoot, _extends({
    ref,
    className: clsx_default(className, classes2.root),
    ownerState: rootProps
  }, other, {
...
var GridColumnHeaders2 = React134.forwardRef(function GridColumnHeaders3(props, ref) {
  const {
    visibleColumns,
    sortColumnLookup,
    filterColumnLookup,
  ...
  return (0, import_jsx_runtime114.jsxs)(GridBaseColumnHeaders, _extends({
    ref
  }, other, getInnerProps(), {
...
var GridNoResultsOverlay = React135.forwardRef(function GridNoResultsOverlay2(props, ref) {
  const apiRef = useGridApiContext();
  const noResultsOverlayLabel = apiRef.current.getLocaleText("noResultsOverlayLabel");
  return (0, import_jsx_runtime115.jsx)(GridOverlay, _extends({
    ref
  }, props, {
...
var GridColumnUnsortedIcon = React136.memo(function GridColumnHeaderSortIcon2(props) {
  const {
    sortingOrder
  ...
  return Icon ? (0, import_jsx_runtime116.jsx)(Icon, _extends({}, other)) : null;
});
...
function MUISelectOption(_ref) {
  ...
  if (native) {
    return (0, import_jsx_runtime117.jsx)("option", _extends({}, props));
  }
  return (0, import_jsx_runtime117.jsx)(MenuItem_default, _extends({}, props));
}
...
var useDataGridProps = (inProps) => {
  ...
  const localeText = React139.useMemo(() => _extends({}, GRID_DEFAULT_LOCALE_TEXT, themedProps.localeText), [themedProps.localeText]);
  const slots = React139.useMemo(() => computeSlots({
    defaultSlots,
    slots: themedProps.slots
  }), [themedProps.slots]);
  return React139.useMemo(() => _extends({}, DATA_GRID_PROPS_DEFAULT_VALUES, themedProps, {
    localeText,
    slots
...
var useGridRowsMeta = (apiRef, props) => {
  ...
  const rowsHeightLookup = React140.useRef(/* @__PURE__ */ Object.create(null));
  const lastMeasuredRowIndex = React140.useRef(-1);
  const hasRowWithAutoHeight = React140.useRef(false);
  const densityFactor = useGridSelector(apiRef, gridDensityFactorSelector);
  const filterModel = useGridSelector(apiRef, gridFilterModelSelector);
  ...
  const hydrateRowsMeta = React140.useCallback(() => {
    var _a, _b;
  ...
  const getRowHeight = React140.useCallback((rowId) => {
  ...
  const setRowHeight = React140.useCallback((id, height) => {
    rowsHeightLookup.current[id].sizes.baseCenter = height;
    rowsHeightLookup.current[id].isResized = true;
    rowsHeightLookup.current[id].needsFirstMeasurement = false;
    hydrateRowsMeta();
  }, [hydrateRowsMeta]);
  const debouncedHydrateRowsMeta = React140.useMemo(() => debounce(hydrateRowsMeta, props.rowPositionsDebounceMs), [hydrateRowsMeta, props.rowPositionsDebounceMs]);
  const storeMeasuredRowHeight = React140.useCallback((id, height) => {
  ...
  const rowHasAutoHeight = React140.useCallback((id) => {
    var _a;
    return ((_a = rowsHeightLookup.current[id]) == null ? void 0 : _a.autoHeight) || false;
  }, []);
  const getLastMeasuredRowIndex = React140.useCallback(() => {
    return lastMeasuredRowIndex.current;
  }, []);
  const setLastMeasuredRowIndex = React140.useCallback((index) => {
    if (hasRowWithAutoHeight.current && index > lastMeasuredRowIndex.current) {
      lastMeasuredRowIndex.current = index;
    }
  }, []);
  const resetRowHeights = React140.useCallback(() => {
...
var useGridStatePersistence = (apiRef) => {
  const exportState = React141.useCallback((params = {}) => {
    const stateToExport = apiRef.current.unstable_applyPipeProcessors("exportState", {}, params);
    return stateToExport;
  }, [apiRef]);
  const restoreState = React141.useCallback((stateToRestore) => {
...
var useGridColumnSpanning = (apiRef) => {
  const lookup = React142.useRef({});
  const getCellColSpanInfo = (rowId, columnIndex) => {
    var _a;
    return (_a = lookup.current[rowId]) == null ? void 0 : _a[columnIndex];
  };
  const calculateColSpan = React142.useCallback(({
    rowId,
    minFirstColumn,
    maxLastColumn,
    columns
  ...
  const handleColumnReorderChange = React142.useCallback(() => {
    lookup.current = {};
  }, []);
...
var useGridColumnGrouping = (apiRef, props) => {
  const getColumnGroupPath = React143.useCallback((field) => {
    const unwrappedGroupingModel = gridColumnGroupsUnwrappedModelSelector(apiRef);
    return unwrappedGroupingModel[field] ?? [];
  }, [apiRef]);
  const getAllGroupDetails = React143.useCallback(() => {
  ...
  const handleColumnIndexChange = React143.useCallback(() => {
    const unwrappedGroupingModel = unwrapGroupingColumnModel(props.columnGroupingModel ?? []);
    apiRef.current.setState((state) => {
      var _a;
  ...
  const updateColumnGroupingState = React143.useCallback((columnGroupingModel) => {
    var _a, _b;
...
var useGridColumnResize = (apiRef, props) => {
  ...
  const colDefRef = React144.useRef();
  const previousMouseClickEvent = React144.useRef();
  const columnHeaderElementRef = React144.useRef();
  const headerFilterElementRef = React144.useRef();
  const groupHeaderElementsRef = React144.useRef([]);
  const cellElementsRef = React144.useRef([]);
  const leftPinnedCellsAfterRef = React144.useRef([]);
  const rightPinnedCellsBeforeRef = React144.useRef([]);
  const fillerLeftRef = React144.useRef();
  const fillerRightRef = React144.useRef();
  const leftPinnedHeadersAfterRef = React144.useRef([]);
  const rightPinnedHeadersBeforeRef = React144.useRef([]);
  const initialOffsetToSeparator = React144.useRef();
  const resizeDirection = React144.useRef();
  const stopResizeEventTimeout = useTimeout();
  const touchId = React144.useRef();
  ...
  const stopListening = React144.useCallback(() => {
    const doc = ownerDocument(apiRef.current.rootElementRef.current);
  ...
  const handleResizeStart = React144.useCallback(({
    field
  }) => {
    apiRef.current.setState((state) => _extends({}, state, {
  ...
  const handleResizeStop = React144.useCallback(() => {
    apiRef.current.setState((state) => _extends({}, state, {
      columnResize: _extends({}, state.columnResize, {
        resizingColumnField: ""
      })
  ...
  const isAutosizingRef = React144.useRef(false);
  const autosizeColumns = React144.useCallback(async (userOptions) => {
    var _a;
  ...
  React144.useEffect(() => stopListening, [stopListening]);
  useOnMount(() => {
...
//# sourceMappingURL=chunk-CJT2HIIB.js.map
